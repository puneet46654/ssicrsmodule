...
export default function Section9({ children }: Section2Props) {
  ...
  useEffect(() => {
    const storageKey = "scroll_boundary_active";
  ...
  useEffect(() => {
    if (!limitActive || !containerRef.current) return;

    const enforceBoundary = () => {
      if (!containerRef.current) return;
      
      const bottomLimit = containerRef.current.offsetTop + containerRef.current.offsetHeight;
      const currentScroll = window.scrollY + window.innerHeight;

      if (currentScroll > bottomLimit) {
        window.scrollTo({
          top: bottomLimit - window.innerHeight,
          behavior: "auto",
        });
      }
    };

    window.addEventListener("scroll", enforceBoundary);
    return () => window.removeEventListener("scroll", enforceBoundary);
  }, [limitActive]);

  return (
    <div ref={containerRef} className="relative w-full">
      <section className="w-full min-h-[85vh] bg-[#FBFAF2] flex flex-col justify-center overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col justify-center py-10 lg:pt-1 lg:pb-16 w-full h-full">
          ...
          >
            <div className="grid grid-cols-2 gap-y-10 gap-x-6 sm:grid-cols-4 sm:gap-x-10 md:gap-x-8 lg:gap-x-12 w-full">
              {logos.map((logo, index) => (
                ...
                >
                  <div className="w-full h-full bg-white rounded-xl shadow-lg flex flex-col items-center p-4 sm:p-6 lg:p-8 transform hover:scale-[1.02] transition-transform duration-300 ease-in-out">
                    <Image
                      src={logo.src}
                      alt={`Logo ${index + 1}`}
                      width={48}
